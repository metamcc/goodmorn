...
To get the `fixtures` folder, you can either follow this command line, which will install and use subversion to get the folder from this repository. Or download the [zip file from Github](https://github.com/chainHero/hyperledger-fabric-server/archive/v1.0.5.zip) and extract only the `fixtures` folder.

```bash
sudo apt install -y subversion && \
cd $GOPATH/src/github.com/chainHero/heroes-service && \
svn checkout https://github.com/chainHero/hyperledger-fabric-server/branches/v1.0.5/fixtures &&
rm -rf fixtures/.svn
```
...
```bash
cd $GOPATH/src/github.com/chainHero/hyperledger-fabric-server/fixtures && \
docker-compose up
```
...
client:
  ...
  cryptoconfig:
    path: "${GOPATH}/src/github.com/chainHero/hyperledger-fabric-server/fixtures/crypto-config"
...
orderers:
  orderer.hf.chainhero.io:
    ...
    tlsCACerts:
      path: "${GOPATH}/src/github.com/chainHero/hyperledger-fabric-server/fixtures/crypto-config/ordererOrganizations/hf.chainhero.io/tlsca/tlsca.hf.chainhero.io-cert.pem"
...
peers:
  peer0.org1.hf.chainhero.io:
    ...
    tlsCACerts:
      path: "${GOPATH}/src/github.com/chainHero/hyperledger-fabric-server/fixtures/crypto-config/peerOrganizations/org1.hf.chainhero.io/tlsca/tlsca.org1.hf.chainhero.io-cert.pem"
...
```bash
mkdir $GOPATH/src/github.com/chainHero/hyperledger-fabric-server/blockchain
```

Now, we add a new go file named `setup.go` :

```bash
vi $GOPATH/src/github.com/chainHero/hyperledger-fabric-server/blockchain/setup.go
```
...
import (
	"fmt"
	"github.com/chainHero/hyperledger-fabric-server/blockchain"
	"os"
)
...
func main() {
	...
	fSetup := blockchain.FabricSetup{
		...
		// Channel parameters 
		ChannelID:       "chainhero",
		ChannelConfig:   os.Getenv("GOPATH") + "/src/github.com/chainHero/hyperledger-fabric-server/fixtures/artifacts/chainhero.channel.tx",
	}
...
- Shut down your network: `cd $GOPATH/src/github.com/chainHero/hyperledger-fabric-server/fixtures && docker-compose down`
...
fSetup := blockchain.FabricSetup{
		...
		ChaincodePath:   "github.com/chainHero/hyperledger-fabric-server/chaincode/",
		OrgAdmin:        "Admin",
		OrgName:         "Org1",
		ConfigFile:      "config.yaml",
