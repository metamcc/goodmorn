...
import (
	...
	"github.com/mycreditchain/eventlistener/eventclient"
	"github.com/mycreditchain/eventlistener/util"
)
...
//	config           string
	...
	// server.key
	clientKey = os.Getenv("HOME") + eventclient.PATH_PREFIX + "/go/src/github.com/mycreditchain/configurations/network-setting/ca-server/fixtures/data/peerOrganizations/org1.hf.mycreditchain.io/peers/peer0.org1.hf.mycreditchain.io/tls/keystore/server.key"
	// server.crt
	clientCert = os.Getenv("HOME") + eventclient.PATH_PREFIX + "/go/src/github.com/mycreditchain/configurations/network-setting/ca-server/fixtures/data/peerOrganizations/org1.hf.mycreditchain.io/peers/peer0.org1.hf.mycreditchain.io/tls/signcerts/server.crt"
	// ca.crt
	rootCert      = os.Getenv("HOME") + eventclient.PATH_PREFIX + "/go/src/github.com/mycreditchain/configurations/network-setting/ca-server/fixtures/data/org1-ca-chain.pem"
	config = "dev" // dev fingerCloud
)
...
func ReadCLInputs() {
	...
	ServerAddrs = []string{"peer0.org1.hf.mycreditchain.io:7051", "peer1.org1.hf.mycreditchain.io:7051", "peer2.org1.hf.mycreditchain.io:7051"}
	...
	flag.StringVar(&config, "config", config, "The config to deliver from.")
	flag.Parse()
...
func InitMSP() {
	// Init the MSP
	var mspMgrConfigDir = os.Getenv("HOME") + eventclient.PATH_PREFIX + "/go/src/github.com/mycreditchain/configurations/network-setting/ca-server/fixtures/data/peerOrganizations/org1.hf.mycreditchain.io/users/admin/msp"
	var mspType = "bccsp"
	var mspID = "org1.hf.mycreditchain.io"
...
func getConfiguration(config string) map[string]interface{} {
	if config != "dev" && config != "local" && config != "fingerCloud" {
		eventclient.Logger.Error("the config flag is wrong")
		return nil
	}
	filename, _ := filepath.Abs(os.Getenv("HOME") + eventclient.PATH_PREFIX + "/go/src/github.com/mycreditchain/eventlistener/config.json")
