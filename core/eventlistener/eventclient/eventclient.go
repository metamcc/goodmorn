...
const (
	...
	CONFIG_FILE = "/go/src/github.com/mycreditchain/echo-rest-server/config/"
	CHANNEL_ID  = "mycreditchain"
	USER_NAME   = "admin"
...
func (r *EventsClient) seekHelper(start *orderer.SeekPosition, stop *orderer.SeekPosition) *common.Envelope {
	env, err := utils.CreateSignedEnvelopeWithTLSBinding(common.HeaderType_DELIVER_SEEK_INFO, "mycreditchain", r.Signer, &orderer.SeekInfo{
		Start:    start,
		Stop:     stop,
...
func (r *EventsClient) ReadEventsStream() {
	...
	for {
		...
		for _, data := range block.GetData().GetData() {
			...
			eventInfos := cAction.GetEvents();

			Logger.Info("event : ", eventInfos, " eventInfos len :  ", len(eventInfos))
			if eventInfos != nil && len(eventInfos) > 0 {

				cEvent, _ := utils.GetChaincodeEvents(eventInfos)
